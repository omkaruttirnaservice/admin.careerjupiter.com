...
const ManageCollege = () => {
    ...
    useEffect(() => {
        const fetchCollegeDetails = async () => {
            ...
            try {
                const response = await axios.get(`${API_BASE_URL}/api/college/${storedCollegeId}`);
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                if (Array.isArray(values.subCategory)) {
                    values.subCategory.forEach((item) => formData.append('subCategory[]', item));
                }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const role = Cookies.get('role');
                        navigate(
                            role === 'ADMIN' ? '/colleges' : '/vendor-college/college-dashboard'
                        );
                    }
                });
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const match = categoryData.find((item) => item.category === selectedCategory);
