...
import { FaEye, FaEdit, FaPauseCircle, FaPlus, FaBuilding, FaBriefcase } from 'react-icons/fa';
...
const UniversityTableDetails = () => {
    ...
    const handleSaveEdit = (updatedData) => {
        ...
        axios
            ...
            .catch((error) => {
                console.error('Error updating university data:', error.response?.data || error);
    ...
    const handleDelete = (item) => {
        ...
        if (isConfirmed) {
            setUniversityData(universityData.filter((university) => university._id !== item._id));
    ...
    const filteredData = universityData.filter((row) => {
        return (
            ...
            `${row.address.line1}, ${row.address.line2}, ${row.address.dist}, ${row.address.state} - ${row.address.pincode}`
                .toLowerCase()
                .includes(searchTerm.toLowerCase()) ||
            row.contactDetails.phoneNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
            row.contactDetails.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
            row.websiteURL.toLowerCase().includes(searchTerm.toLowerCase()) ||
            row.establishedYear.toString().includes(searchTerm) ||
            (row.accreditation || []).join(' ').toLowerCase().includes(searchTerm.toLowerCase())
        );
    });
    ...
    const columns = [
        ...
        {
            name: 'Contact',
            selector: (row) => `${row.contactDetails.phoneNumber}, ${row.contactDetails.email}`,
            sortable: true,
        },
        ...
        <section>
            <div className="bg-gradient-to-r from-blue-50 to-blue-100 min-h-screen p-6 shadow-lg rounded-lg">
                <div className="flex justify-between items-center bg-white p-4 shadow-md rounded-lg mb-4">
                    <h2 className="text-3xl font-semibold text-blue-800">🏛 University List</h2>
                    <div className="ml-4">
                        <input
                            type="text"
                ...
                {modalOpen && (
                    <UniversityInfoCard universityData={selectedItem} onClose={handleCloseModal} />
                )}
