...
//                 className="hidden"
//                 onChange={handleImageLogo} // ✅ Now calling the function
//               />
...
const LocationMarker = ({ setLocation, location, setFieldValue }) => {
  useMapEvents({
    click(e) {
      ...
      setLocation({ lat, lan: lng })
      setFieldValue("lat", lat)
      setFieldValue("lan", lng)
    },
  })
...
const AddNewCollege = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        formData.append("collegeName", values.collegeName)
        formData.append("affiliatedUniversity", values.affiliatedUniversity)
        formData.append("Category", values.collegeCategory || "General")
        formData.append("collegeType", values.collegeType)
  ...
  const handleSearch = async () => {
    try {
      ...
      if (data.length > 0) {
        const { lat, lon } = data[0]
        const newLocation = { lat: Number.parseFloat(lat), lan: Number.parseFloat(lon) }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full px-4 py-2 border rounded-md shadow-sm focus:ring-2 focus:ring-blue-500"
              />
              {formik.touched.admissionProcess && formik.errors.admissionProcess && (
                <p className="text-red-500 text-sm">{formik.errors.admissionProcess}</p>
              )}
            </div>
