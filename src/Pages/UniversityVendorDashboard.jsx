    ...
    useEffect(() => {
        const fetchUniversityDetails = async () => {
            ...
            try {
                ...
                console.error(
                    'Error fetching university details:',
                ...
                <div className="text-center sm:text-left">
                    <h2 className="text-3xl font-extrabold text-gray-800 mb-1">
                        ...
                        <span className="text-green-600">
                            {universityDetails?.universityName || 'University Vendor'}
                        </span>{' '}
                        👋
                    </h2>
                    <p className="text-lg text-gray-700 font-semibold">
                        {universityDetails?.address?.state || 'State Info'}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white/80 shadow-xl p-4 rounded-2xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {[
                        ...
                        {
                            title: 'Address',
                            value: `${universityDetails?.address?.line1 || ''}, ${
                                universityDetails?.address?.line2 || ''
                            }, ${universityDetails?.address?.dist || ''}`,
                ...
                <div className="bg-gradient-to-br from-yellow-100 to-orange-200 p-6 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            {new Date(
                                universityDetails?.admissionEntranceDetails?.admissionStartDate
                            ).toLocaleDateString() || 'N/A'}
                        </div>
                        <div>
                            <strong>End Date:</strong>{' '}
                            {new Date(
                                universityDetails?.admissionEntranceDetails?.admissionEndDate
                            ).toLocaleDateString() || 'N/A'}
                        </div>
                        <div>
                            <strong>Last Year Cutoff Marks:</strong>{' '}
                            {universityDetails?.admissionEntranceDetails?.lastYearCutoffMarks ||
                                'N/A'}
                        </div>
                        <div>
                            <strong>Scholarships Available:</strong>{' '}
                            {universityDetails?.admissionEntranceDetails?.scholarshipsAvailable?.join(
                                ', '
                            ) || 'N/A'}
                        </div>
                        <div>
                            <strong>Quota System:</strong>{' '}
                            {universityDetails?.admissionEntranceDetails?.quotaSystem?.join(', ') ||
                                'N/A'}
                        </div>
                    </div>
                </div>
