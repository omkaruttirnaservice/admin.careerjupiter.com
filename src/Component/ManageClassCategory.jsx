...
//   //     const classCategories = allCategories.filter((cat) => cat.type === "class");
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (res.data) {
        ...
        setCategories((prev) => ({
          ...prev,
          [type]: tempCategories, // 🔥 here type = "class"
        }));
      }
    } catch (error) {
      console.error("Error fetching categories:", error);
...
const handleAddCategory = async () => {
  try {
    ...
    // If successful, update the categories state to include the new category
    const newCategory = {
      _id: res.data.data._id,  // Assuming the response contains the new category data
      category: categoryInput,
    };
    ...
    setCategories((prevCategories) => ({
      ...prevCategories,
      [type]: [...prevCategories[type], newCategory],  // Add the new category to the existing list
    }));
...
const handleDeleteCategory = async (id) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      // If user cancels, just show a cancel message (optional)
      Swal.fire({
        icon: 'info',
        title: 'Cancelled',
