...
const ManageUniversityCategory = ({ onCategoriesChange }) => {
    ...
    const fetchCategories = async () => {
        try {
            const res = await axios.get(`${API_BASE_URL}/api/university/categories/all`);
    ...
    const handleAddCategory = async () => {
        ...
        const subcategories = subCategoryInputs.map((sub) => sub.trim()).filter((sub) => sub);
                                                ...
                                                <div key={index} className="flex gap-2">
                                                    ...
                                                    {editSubCategories.length > 1 && (
                                                        <button
                                                            className="bg-red-400 hover:bg-red-500 text-white p-2 rounded-lg"
                                                            onClick={() =>
                                                                handleRemoveEditSubCategoryField(
                                                                    index
                                                                )
                                                            }>
                                                            <Trash2 size={18} />
