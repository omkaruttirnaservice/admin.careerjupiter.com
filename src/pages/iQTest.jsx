...
const IQTest = () => {
  ...
  // Adding Category is Handled over here
  const handleAddCategory = async (e) => {
    e.preventDefault();

    if (!mainCategory.trim() || !description.trim()) return;

    // Validation for Main category section accepts only (letter, number, space and -)
    ...
    try {
      ...
      // Set Each field empty when added successfully
      setMainCategory("");
      setDescription("");
      setShowModal(false);
      fetchCategories();
  ...
  // handled deletion of categories
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {/* Card - shows main category and description */}
          ...
          categories.map((cat, index) => (
            ...
            >
              ...
              >
                <Trash2
                  size={16}
                  className="transition-transform duration-200 group-hover:rotate-360"
                />
              </button>
            </div>
          ))}
          ...
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[90%] max-w-md">
            ...
            <form className="space-y-4" onSubmit={handleAddCategory}>
              ...
              <div className="flex justify-end gap-3">
                {/* Cancel Button - clear all fields and close the modal*/}
                <button
                  type="button"
                  onClick={() => {
                    setShowModal(false);
